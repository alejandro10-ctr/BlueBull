window.addEventListener('load', function(){
    ...
    class Enemy {
        ...
        draw(context){
            context.drawImage(this.image, this.frameX * this.spriteX, this.frameY * this.spriteHeight, this.spriteWidth, this.spriteHeight, this.spriteX, this.spriteY, this.width, this.height);
            if (this.game.debug){
                context.beginPath();
                context.arc(this.collisionX, this.collisionY, this.collisionRadius, 0, Math.PI * 2);
                context.save();
    ...
    class Game {
        ...
        render(context, deltaTime){
            ...
            context.fillText('Lost: ' + this.lostHatchlings, 1125, 50);
            
            context.restore();

            // win / lose message
            if(this.score >= this.winningScore || this.lostHatchlings === 25){
              this.gameOver = true;  
              context.save();
              ...
              let message1;
              let message2;
              if (this.lostHatchlings < 25){
                  message1 = "You Win!!!"
                  message2 = "Congrats!!"  // "You bullied the bullies!"
              } else {
                //lose
                
                message1 = "You Loose!!"
                message2 = this.lostHatchlings + " hatchlings lost =(" 
              }
        ...
        init(){
            ...
            while (this.obstacles.length < this.numberOfObstacles && attempts < 500){
                let testeObstacle = new Obstacle(this)
                let overlap = false;
                this.obstacles.forEach(obstacle => {
                    const dx = testeObstacle.collisionX - obstacle.collisionX;
                    const dy = testeObstacle.collisionY - obstacle.collisionY;
                    const distance = Math.hypot(dy, dx);
                    const distanceBuffer = 150;
                    const sumOfRadii = testeObstacle.collisionRadius + obstacle.collisionRadius;
                    if (distance < sumOfRadii){
                        overlap = true;
                    }
                });
                const margin = testeObstacle.collisionRadius * 3;
                if(!overlap && testeObstacle.spriteX > 0 && testeObstacle.spriteX < this.width - testeObstacle.width && 
                    testeObstacle.collisionY > this.topMargin + margin && testeObstacle.collisionY < this.height - margin){
                    this.obstacles.push(testeObstacle);
                }
                attempts++;
